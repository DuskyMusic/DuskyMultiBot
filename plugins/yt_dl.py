...
def song(client, message):
    ...
    try:
        ...
        m.edit(
            "**Found Nothing Please Correct The Spelling (Or) Search Any Other Song**"
        )
        print(str(e))
        return
    ...
    try:
        ...
        rep = '**By›› [DuskyMultiBot](https://github.com/DuskyMusic/DuskyMultiBot)**'
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(
        message.chat.id, f"**Finding Your Video** `{urlissed}`"
    )
    if not urlissed:
    ...
    try:
        ...
        await event.edit(event, f"**Download Failed Please Try Again!** \n**Error :** `{str(e)}`")
        return
    c_time = time.time()
