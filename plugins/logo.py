...
async def logosq(bot, message):
    ...
    if FSub == 400:
        return 
    try:
      text = message.text.replace("logosq","").replace("/","").replace("[Dusky]","").strip().upper()
      ...
      if "graph.org" not in logo:
        return await x.edit("`❌ Something Went Wrong...`\n\nReport This Error In [SUPPORT GRP](t.me/DuskysSupport).")
...
async def logo(bot, message):
  ...
  if FSub == 400:
      return 
  try:
    text = message.text.replace("logo","").replace("/","").replace("@DuskyMultiBot","").strip().upper()
    ...
    logo_id = logo.replace("https://graph.org//file/","").replace(".jpg","").strip()
    await message.reply_photo(logo,caption = "**🖼 Logo Generated By DuskyMultiBot**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
  except Exception as e:
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By [Dusky]**")
  except FloodWait:
    pass
  except Exception as e:
    try:
